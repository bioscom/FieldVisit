<%@ Control Language="C#" AutoEventWireup="true" CodeFile="oRequestsPendingMyAction.ascx.cs" Inherits="App_BI500_UserControl_Cost_oRequestsPendingMyAction" %>
...
<asp:GridView ID="grdGridView" runat="server" AllowPaging="True"
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="">
            <ItemTemplate>
                <asp:LinkButton ID="ActionLinkButton" runat="server" ValidationGroup="Xpx"
                    CommandArgument="<%# Container.DisplayIndex %>" CommandName="Action"
                    STATUS='<%# DataBinder.Eval(Container.DataItem, "STATUS") %>'
                    IDREQUEST='<%# DataBinder.Eval(Container.DataItem, "IDREQUEST") %>'>Action</asp:LinkButton>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="">
            <ItemTemplate>
                <asp:LinkButton ID="AssignApproversLinkButton" runat="server" ValidationGroup="Xpx"
                    ...
                    STATUS='<%# DataBinder.Eval(Container.DataItem, "STATUS") %>'
                    IDREQUEST='<%# DataBinder.Eval(Container.DataItem, "IDREQUEST") %>'>Assign Managers</asp:LinkButton>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="Focal Point" SortExpression="FOCALPOINTFULLNAME">
            <ItemTemplate>
                <asp:Label ID="labelFocalPoint" runat="server" FOCALPOINTUSERID='<%# DataBinder.Eval(Container.DataItem, "FOCALPOINTUSERID") %>' Text='<%# DataBinder.Eval(Container.DataItem, "FOCALPOINTFULLNAME") %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="Workstream Owner" SortExpression="CHAMPIONFULLNAME">
            <ItemTemplate>
                <asp:Label ID="labelChampion" runat="server" CHAMPIONUSERID='<%# DataBinder.Eval(Container.DataItem, "CHAMPIONUSERID") %>' Text='<%# DataBinder.Eval(Container.DataItem, "CHAMPIONFULLNAME") %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>

        <asp:TemplateField HeaderText="Workstream Sponsor" SortExpression="SPONSORFULLNAME">
            <ItemTemplate>
                <asp:Label ID="lblSponsor" runat="server" SPONSORUSERID='<%# DataBinder.Eval(Container.DataItem, "SPONSORUSERID") %>' Text='<%# DataBinder.Eval(Container.DataItem, "SPONSORFULLNAME") %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="Status">
            <ItemTemplate>
                <asp:Label ID="labelStatus" runat="server" STATUS='<%# DataBinder.Eval(Container.DataItem, "STATUS") %>' Text='<%# DataBinder.Eval(Container.DataItem, "STATUS") %>'></asp:Label>
            </ItemTemplate>
    ...
    <asp:Panel ID="pnlDragPanel" runat="server" Style="cursor: move; background-color: #DDDDDD; border: solid 1px Gray; color: Black">
        <div style="width: 805px">
            ...
            <div style="float: right">
                <asp:Button ID="closeButton" runat="server" Text="Close" ValidationGroup="xxxx" Width="100px" /></div>
        </div>
    </asp:Panel>
    <br />
    <div style="width: 805px">
