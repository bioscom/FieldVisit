<%@ Control Language="C#" AutoEventWireup="true" CodeFile="oMyPendingRequests.ascx.cs" Inherits="App_IDD_UserControl_oMyPendingRequests" %>
...
<telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
    <script type="text/javascript">
        ...
        function ShowInsertFormPending() {
            var wndw = window.radopen("MakeRequestNew.aspx", "InputDialog", 750, 500, "Create");
            wndw.SetTitle("Make IDD Request");
            wndw.set_visibleStatusbar(false);
            wndw.Center();
            return false;
        }

        function ShowEditFormPending(id, rowIndex) {
            var grid = $find("<%= grdView.ClientID %>");

            var rowControl = grid.get_masterTableView().get_dataItems()[rowIndex].get_element();
            grid.get_masterTableView().selectItem(rowControl, true);

            var wndw = window.radopen("EditRequest.aspx?lRequestId=" + id, "EditDialog", 750, 500, "Edit");
            wndw.SetTitle("Edit/Update Request");
            wndw.set_visibleStatusbar(false);
            wndw.Center();
            return false;
        }

        function ShowActionFormPending(id, rowIndex) {
            var grid = $find("<%= grdView.ClientID %>");

            var rowControl = grid.get_masterTableView().get_dataItems()[rowIndex].get_element();
            grid.get_masterTableView().selectItem(rowControl, true);

            var wndw = window.radopen("CPFocalPointAction.aspx?lRequestId=" + id, "ActionDialog", 650, 300);
            wndw.SetTitle("Focal Point Action");
            wndw.set_visibleStatusbar(false);
            wndw.Center();
            return false;
        }
...
<div id="demo" class="demo-container no-bg" style="width: 99%">
    ...
    <telerik:RadGrid RenderMode="Lightweight" ID="grdView" AllowSorting="true" runat="server" PageSize="15" AllowAutomaticInserts="true"
        ...
        <MasterTableView Width="100%" CommandItemDisplay="None" DataKeyNames="REQUESTID, VENDORID" AutoGenerateColumns="false" InsertItemDisplay="Top" InsertItemPageIndexAction="ShowItemOnFirstPage">
            ...
            <CommandItemTemplate>
                 <center><a href="#" onclick="return ShowInsertFormPending();"><b style="color:red; font-size:14px">Create New Request</b></a></center>
            </CommandItemTemplate>
    ...
    <telerik:RadWindowManager RenderMode="Lightweight" ID="RadWindowManager1" runat="server" EnableShadow="true">
        <Windows>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="InputDialog" runat="server" Title="Create New Request" Height="500px" Width="750px"
                Left="50px" ReloadOnShow="true" ShowContentDuringLoad="false" Modal="true">
            </telerik:RadWindow>

            <telerik:RadWindow RenderMode="Lightweight" ID="EditDialog" runat="server" Title="Update Request" Height="500px" Width="750px"
                Left="50px" ReloadOnShow="true" ShowContentDuringLoad="false" Modal="true">
            </telerik:RadWindow>

            <telerik:RadWindow RenderMode="Lightweight" ID="ActionDialog" runat="server" Title="Action Integrity Due Diligence Request" Height="300px" Width="650px"
                Left="50px" ReloadOnShow="true" ShowContentDuringLoad="false" Modal="true">
            </telerik:RadWindow>
