...
public partial class App_PPMS_EditActionTrackerForm : aspnetPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (String.IsNullOrEmpty(Request.QueryString["ActionId"])) return;
            long lActionId = long.Parse(Request.QueryString["ActionId"]);
            LoadProject(lActionId);
        }
    }
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        oActionTracker.lActionId = long.Parse(Request.QueryString["ActionId"]);

        bool bRet = ActionTrackerRegisterHelper.UpdateActionTracker(oActionTracker);
    ...
    private void LoadProject(long lActionId)
    {
        var oActionTrackerRegister = ActionTrackerRegisterHelper.ObjGetActionById(lActionId);

        ddlActionParty.thisDropDownList.SelectedValue = oActionTrackerRegister.ActionParty.m_iUserId.ToString();
        txtActivities.Text = oActionTrackerRegister.Activities;
        txtComments.Text = oActionTrackerRegister.Comments;
        txtNextSteps.Text = oActionTrackerRegister.NextSteps;
        txtAction.Text = oActionTrackerRegister.sAction;

        ddlImportance.SelectedValue = oActionTrackerRegister.Importance.ToString();
        ddlStatus.SelectedValue = oActionTrackerRegister.ProjectStatus.ToString();
        ddlCategory.SelectedValue = oActionTrackerRegister.Tcategory.CategoryId.ToString();
        ddlAsset.SelectedValue = oActionTrackerRegister.TfunctionAsset.AssetId.ToString();
        ddlTheme.SelectedValue = oActionTrackerRegister.Theme.ThemeId.ToString();
        ddlUrgency.SelectedValue = oActionTrackerRegister.Urgency.ToString();
        dtCloseOutDate.dtSetDate(oActionTrackerRegister.DateClosedOut);
    }
